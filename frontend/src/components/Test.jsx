...
export default function Test() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.REACT_APP_API_URL}/test`, {
        method: "POST",
        body: formData,
      });
...
<main className="min-h-screen flex flex-col justify-between max-w-4xl mx-auto px-4 py-6">
        <div className="bg-white/50 backdrop-blur-lg py-8 px-8 rounded-2xl shadow-md w-full text-center">
          {geminiHTML ? (
